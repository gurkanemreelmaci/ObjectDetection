...
class ObjectTracker(Model): 
    ...
    def train_step(self, batch, **kwargs): 
        ...
        return {"total_loss":total_loss, "class_loss":batch_classloss, "regress_loss":batch_localizationloss,"total_accuracy":total_accuracy}
...
def build_model(): 
    ...
    siamese = model = keras.Sequential(
    [
    # ilk blok
    ...
    # sınıflandırma  
    f1 = GlobalMaxPooling2D()(siamese)
    class1 = Dense(2048, activation='relu')(f1)
    
    class2 = Dense(1, activation='softmax')(class1)
    
    # kordinat eşleme
    f2 = GlobalMaxPooling2D()(siamese)
...
train_labels = train_labels.map(lambda x: tf.py_function(fp.load_labels, [x], [tf.uint8, tf.float16]))
...
model.compile(opt, classloss, regressloss,accuracy)
...
objecttracker.save('objecttracker_siemase.h5')
